...
class z.ViewModel.AuthViewModel
  ...
  sign_in_password: =>
    return if @pending_server_request() or not @_validate_input z.auth.AuthView.MODE.ACCOUNT_PASSWORD
    ...
    .then =>
      amplify.publish z.event.WebApp.ANALYTICS.EVENT, z.tracking.EventName.ACCOUNT.LOGGED_IN,
        {
          context: z.auth.AuthView.MODE.ACCOUNT_PASSWORD
          remember_me: @persist()
        }
  ...
  verify_account: =>
    return if @pending_server_request() or not @_validate_input z.auth.AuthView.MODE.VERIFY_ACCOUNT
  ...
  verify_password: =>
    ...
    .then =>
      amplify.publish z.event.WebApp.ANALYTICS.EVENT, z.tracking.EventName.ACCOUNT.LOGGED_IN,
        {
          context: z.auth.AuthView.MODE.VERIFY_PASSWORD
          remember_me: @persist()
        }
      @_authentication_successful()
    .catch (error) =>
  ...
  _shift_ui: (animation_params) =>
    ...
    if not animation_params.direction
      ...
      _change_visible()
    else if not old_component.length
  ...
  _on_hash_change: =>
    ...
    switch @_get_hash()
      ...
      when z.auth.AuthView.MODE.ACCOUNT_PASSWORD then @show_account_password()
      when z.auth.AuthView.MODE.ACCOUNT_PHONE then @show_account_phone()
      when z.auth.AuthView.MODE.VERIFY_ACCOUNT then @show_verify_account()
      when z.auth.AuthView.MODE.VERIFY_CODE then @show_verify_code()
      when z.auth.AuthView.MODE.VERIFY_PASSWORD then @show_verify_password()
